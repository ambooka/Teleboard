...
import com.msah.teleboard.boards.ImageSelectorActivity;
import com.msah.teleboard.boards.adapters.FolderAdapter;
import com.msah.teleboard.boards.adapters.ImageGridAdapter;
import com.msah.teleboard.boards.models.Folder;
import com.msah.teleboard.boards.models.Image;
import com.msah.teleboard.customLoadingView.DensityUtil;
import com.msah.teleboard.utils.FileUtils;
import com.msah.teleboard.boards.models.Image;
...
public class ImageSelectorFragment extends Fragment {
    ...
    private void getRequstType() {
        mRequestType = getArguments().getInt(ImageSelectorActivity.EXTRA_REQUEST_TYPE);
    }
...
//        mFolderPopupWindow.setHeight(height);
    ...
    private void setPopupWindowSize(int orientation) {
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {//横屏
            int screenWidth = Math.max(BoardActivity.mBoardWidth, BoardActivity.mBoardWidth);
            int screenHight = Math.min(BoardActivity.mBoardHeight, BoardActivity.mBoardWidth);
            mFolderPopupWindow.setWidth((int) (screenWidth / 2 - DensityUtil.dip2px(getActivity(), 60)));
            mFolderPopupWindow.setHeight(screenHight / 2);
        } else if (orientation == Configuration.ORIENTATION_PORTRAIT) {//竖屏
            int screenWidth = Math.min(BoardActivity.mBoardHeight, BoardActivity.mBoardWidth);
            int screenHight = Math.max(BoardActivity.mBoardHeight, BoardActivity.mBoardWidth);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CAMERA) {
            if (resultCode == Activity.RESULT_OK) {
                if (mTmpFile != null) {
                    if(mRequestType==BoardActivity.REQUEST_IMAGE) {//如果是拍照作为画板素材的话
                        cropPhoto(Uri.fromFile(mTmpFile));// 裁剪图片
                    }else if(mRequestType==BoardActivity.REQUEST_BACKGROUND) {//拍照作为画板背景
                        if (mCallback != null) {
